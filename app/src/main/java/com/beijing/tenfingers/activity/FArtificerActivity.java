...
//技师列表直接查看技师详情 然后跳转到 项目选择列表
public class FArtificerActivity extends BaseActivity implements View.OnClickListener , ZanClickListener {
...
//                getNetWorker().tech_detail(id);
    ...
    protected void callBackForServerSuccess(HemaNetTask netTask, HemaBaseResult baseResult) {
        ...
        switch (information) {
            ...
            case TECHICIAN_DETAIL:
                ...
                RadarView.borderCount=detail.getCountLists().size();
    ...
    private void initData(){
        ...
        chart.setExtraOffsets(30, 10, 30, 5);
        ...
        chart.getDescription().setEnabled(false); //是否取消右下角描述
        chart.setDrawEntryLabels(false);
        setChartData(countLists.size());
    ...
    private void setChartData(int count) {
        ...
        if(countLists!=null){
            if(countLists.size()>0){
                parties = new String[countLists.size()];
                data2 = new float[countLists.size()];
                for (int i = 0; i < countLists.size(); i++) {
                    parties[i]=countLists.get(i).getP_name();
                    data2[i]=Integer.valueOf(countLists.get(i).getTotal());
                }
            }
        }
        ...
        for (int i = 0; i < count ; i++) {
            entries.add(new PieEntry(data2[i], parties[i], getResources().getDrawable(R.drawable.star)));
        }
...
//                String path = MyConfig.WEB_ROOT;
            ...
            case R.id.tv_book://立即预约
                if(BaseUtil.IsLogin()){
                    it=new Intent(mContext, ServiceListActivity.class);
                    it.putExtra("tid",id);
                    startActivity(it);
                    changeAc();
                }else{
    ...
    private SpannableString generateCenterSpannableText() {
        ...
        SpannableString s = new SpannableString("技师接单统计");
