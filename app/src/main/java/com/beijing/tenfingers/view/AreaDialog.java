...
import com.beijing.tenfingers.bean.CityChildren0;
import com.beijing.tenfingers.wheelview.ArrayWheelAdapter;
import com.beijing.tenfingers.wheelview.OnWheelChangedListener;
import com.beijing.tenfingers.wheelview.WheelView;
...
public class AreaDialog extends XtomObject implements OnWheelChangedListener {
    ...
    private void initProvinceDatas() {
        ...
        if (provinceList != null && !provinceList.isEmpty()) {
            ...
            List<CityChildren0> cityList = provinceList.get(0).getChildren();
            if (cityList != null && !cityList.isEmpty()) {
                mCurrentCityName = cityList.get(0).getName();
                List<CityChildren0> districtList = cityList.get(0).getChildren();
        ...
        for (int i = 0; i < provinceList.size(); i++) {
            ...
            List<CityChildren0> cityList = provinceList.get(i).getChildren();
            String[] cityNames = new String[cityList.size()];
            for (int j = 0; j < cityList.size(); j++) {
                // 遍历省下面的所有市的数据
                cityNames[j] = cityList.get(j).getName();// 当前省
                List<CityChildren0> districtList = cityList.get(j).getChildren();// 当前省下的城市
                String[] distrinctNameArray = new String[districtList.size()];// 城市名称列表
                CityChildren0[] distrinctArray = new CityChildren0[districtList
                        .size()];// 城市列表
                for (int k = 0; k < districtList.size(); k++) {
                    // 遍历市下面所有区/县的数据
                    CityChildren0 districtModel = districtList.get(k);
    ...
    private void updateAreas() {
        ...
        if (mCitisDatasMap.get(mCurrentProviceName) == null || mCitisDatasMap.get(mCurrentProviceName).length == 0) {
            ...
            id_district.setViewAdapter(new ArrayWheelAdapter<String>(context, areas));
    ...
    public String getArea() {
        ...
        if (mCurrentCityName.equals("县")) {
            returnStr = mCurrentProviceName + mCurrentDistrictName;
        } else {

            returnStr = mCurrentProviceName + mCurrentCityName + mCurrentDistrictName;
        }
        return returnStr;
    }

    public String getDivideArea() {
    ...
    public String getProvinceId(){
        if (city.getChildren() != null
                && city.getChildren().size() > mCurrentProvicePosition) {
            return  city.getChildren().get(mCurrentProvicePosition).getId();
        }
        return "";
    }
    ...
    public String getCityId(){
        if (city.getChildren().get(mCurrentProvicePosition).getChildren() != null
                && city.getChildren().get(mCurrentProvicePosition).getChildren().size() > mCurrentCityPosition){
            return  city.getChildren().get(mCurrentProvicePosition).getChildren().get(mCurrentCityPosition).getId();
        }
        return "";
    }
    ...
    public String getDistinctId(){
        if (city.getChildren().get(mCurrentProvicePosition).getChildren().get(mCurrentCityPosition).getChildren() != null
                && city.getChildren().get(mCurrentProvicePosition).getChildren().get(mCurrentCityPosition).getChildren().size() > mCurrentDistrictPosition){
            return  city.getChildren().get(mCurrentProvicePosition).getChildren().get(mCurrentCityPosition)
                    .getChildren().get(mCurrentDistrictPosition).getId();
        }
        return "";
    }
