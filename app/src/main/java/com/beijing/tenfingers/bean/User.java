...
public class User extends HemaUser implements Serializable {
	...
	private String selfsign;//用户身份
	private String realname;// 姓名
	private String company_id;// 所属公司主键 根据此值是否为空来判断是普通用户还是企业用户
	private String dept_id;// 所属部门id
	private String worker;//用户状态
	private String sealflag;
	private String adminflag;
	private String viptype;//所属医院id
	private String vipenddate;// 所属医院名称
	private String level;// 所属科室id
	private String score;// 积分余额
	...
	public User(JSONObject jsonObject) throws DataParseException {
		...
		if (jsonObject != null) {
			try {
				...
				selfsign = get(jsonObject, "clienttype");
				realname = get(jsonObject, "realname");
				company_id = get(jsonObject, "csc_user_id");//客服id
				dept_id = get(jsonObject, "depart_id");
				worker = get(jsonObject, "status");
				sealflag = get(jsonObject, "sealflag");
				adminflag = get(jsonObject, "adminflag");
				viptype = get(jsonObject, "is_vip");
				vipenddate = get(jsonObject, "hospital_name");
				level = get(jsonObject, "room_id");
				score = get(jsonObject, "score");
				feeaccount = get(jsonObject, "feeaccount");
				avatar = get(jsonObject, "img_link");
				avatarbig = get(jsonObject, "avatarbig");
				district_name = get(jsonObject, "district_name");
				validflag = get(jsonObject, "is_old");//是否老用户 1是 0否
				...
				companyname = get(jsonObject, "company");
				deptname = get(jsonObject, "wx_uid");//微信UID
				away_score = get(jsonObject, "custom_token");//qquid
