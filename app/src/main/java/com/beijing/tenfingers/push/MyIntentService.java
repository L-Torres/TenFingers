...
public class MyIntentService extends GTIntentService {
    ...
    public void onReceiveMessageData(Context context, GTTransmitMessage msg) {
        ...
        Log.d(TAG, "onReceiveMessageData -> " + "appid = " + appid + "\ntaskid = " + taskid + "\nmessageid = " + messageid + "\npkg = " + pkg
                + "\ncid = " + cid + ", payload = " + msg.getPayload().toString());
        ...
        if (payload == null) {
            ...
            try {
                ...
                if(msgJson.has("target_type")){
                    keyType = msgJson.getString("target_type");
                }
                if(msgJson.has("notify_target_id")){
                    keyId = msgJson.getString("notify_target_id");
                }
                if(msgJson.has("msg")){
                    msg_content = msgJson.getString("msg");
                }
                if(msgJson.has("title")){
                    title = msgJson.getString("title");
                }
                if (msgJson.has("notify_target_id")) {
                    targetid = msgJson.getString("notify_target_id");
                }
                if (msgJson.has("web_title")) {
                    web_title = msgJson.getString("web_title");
                    web_link = msgJson.getString("web_link");
                }

                PushModel currentPushModel = new PushModel(keyType, keyId, msg_content, "", targetid, web_title, web_link, id, title);
                sendMessage(currentPushModel, 1);
    ...
    public void onReceiveClientId(Context context, String clientid) {
        EventBus.getDefault().post(new EventBusModel(true, MyEventBusConfig.PSUH_SUCCESS, clientid));
    }
