...
public class JsxmAdapter extends BaseRecycleAdapter<Product> {
    ...
    public JsxmAdapter(Context contex, ArrayList<Product> datas) {
        super(datas);
        this.context = contex;
        this.list = datas;
    }
...
//        ((TextView) holder.getView(R.id.tv_tag)).setVisibility(View.GONE);
        ...
        BaseUtil.loadBitmap(list.get(position).getP_image_link(),0, (ImageView)holder.getView(R.id.iv_img));
        ((TextView)holder.getView(R.id.tv_name)).setText(list.get(position).getP_name());
        ((TextView)holder.getView(R.id.tv_time)).setText(list.get(position).getCreated_at());
