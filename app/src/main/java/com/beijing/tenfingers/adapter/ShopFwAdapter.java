...
import com.beijing.tenfingers.shop.ShopProDetailActivity;
...
//店铺详情内的服务项目
public class ShopFwAdapter extends BaseRecycleAdapter<Product> {

    private ArrayList<Product> list = new ArrayList<>();
    private Context context;

    public ShopFwAdapter(Context contex, ArrayList<Product> datas) {
        super(datas);
        this.context = contex;
        this.list = datas;
    }
...
//        ((TextView) holder.getView(R.id.tv_tag)).setVisibility(View.GONE);
        ...
        if(!list.get(position).getP_service_time().contains("h")){
            ((TextView)holder.getView(R.id.tv_time)).setText(BaseUtil.toHour(list.get(position).getP_service_time()));
        }else{
        ...
        if("1".equals(list.get(position).getIs_activity())){
            ...
            ((TextView)holder.getView(R.id.tv_dis_price)).setVisibility(View.VISIBLE);
        }else{
        ...
        holder.getView(R.id.tv_book).setVisibility(View.INVISIBLE);
...
//        });
        ...
        holder.getView(R.id.v_all).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Product p= (Product) v.getTag();
                Intent it=new Intent(context, ShopProDetailActivity.class);
                it.putExtra("id",p.getId());
