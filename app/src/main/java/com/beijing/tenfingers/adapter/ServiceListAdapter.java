...
import com.beijing.tenfingers.activity.SelProductDetailActivity;
...
public class ServiceListAdapter extends BaseRecycleAdapter<Product> {

    private ArrayList<Product> list = new ArrayList<>();
    private Context context;
    private String tid;
    public ServiceListAdapter(Context contex, ArrayList<Product> datas,String tid) {
        super(datas);
        this.context = contex;
        this.list = datas;
        this.tid=tid;
...
//        ((TextView) holder.getView(R.id.tv_tag)).setVisibility(View.GONE);
        ...
        if("1".equals(list.get(position).getIs_activity())){
            ...
            ((TextView)holder.getView(R.id.tv_dis_price)).setText(list.get(position).getP_price());
        ...
        holder.getView(R.id.v_all).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Product p= (Product) v.getTag();
                Intent it=new Intent(context, SelProductDetailActivity.class);
                it.putExtra("technicianId",tid);
                it.putExtra("id",p.getId());
                it.putExtra("sid",p.getSid());
                context.startActivity(it);
