 ...
 * 
 */
public abstract class TorresFragmentActivity extends FragmentActivity {
    ...
    protected TorresFragmentActivity() {
    }
    ...
    public void getDataFromServer(BaseNetTask task) {
        if (this.netWorker == null) {
            ...
            this.netWorker.setOnTaskExecuteListener(new TorresFragmentActivity.OnTaskExecuteListener((TorresFragmentActivity.OnTaskExecuteListener)null));
        }
    ...
    private class OnTaskExecuteListener implements BaseNetWorkerNew.OnTaskExecuteListener {
        ...
        public void onPreExecute(BaseNetWorkerNew netWorker, BaseNetTask task) {
            TorresFragmentActivity.this.callBeforeDataBack(task);
        }

        public void onPostExecute(BaseNetWorkerNew netWorker, BaseNetTask task) {
            TorresFragmentActivity.this.callAfterDataBack(task);
        }

        public void onExecuteSuccess(BaseNetWorkerNew netWorker, BaseNetTask task, Object result) {
            TorresFragmentActivity.this.callBackForGetDataSuccess(task, result);
        }

        public void onExecuteFailed(BaseNetWorkerNew netWorker, BaseNetTask task, int failedType) {
            switch(failedType) {
                case -4:
                    TorresFragmentActivity.this.noNetWork(task);
                    break;
                case -3:
                case -2:
                    TorresFragmentActivity.this.callBackForGetDataFailed(failedType, task);
            }
